  ...
  async execute(interaction: ChatInputCommandInteraction<CacheType>) {
    ...
    if (member_matches.length < 1) {
      await interaction
        .editReply({
          content:
            "I don't have you registered. Try running the register command first.",
        })
        .then(() => {
          delayDelete(interaction, 60000);
        });
      ...
      await interaction
        .editReply({
          content:
            "Somehow too many members are registered to this discord ID. See the database administrator.",
        })
        .then(() => {
          delayDelete(interaction, 60000);
        });
    ...
    for (let r of member.roles_held) {
      ...
      if (
        role_data?.access_level == AccessLevel.ADMIN ||
        role_data?.access_level == AccessLevel.MOD
      ) {
        has_authority = true;
        break;
      }
    ...
    await interaction
      .editReply({
        content:
          "You have authority to assign roles, but it hasn't yet been implemented.",
      })
      .then(() => delayDelete(interaction, 60000));
  }
}
